...
void initnRF24L01(uint8_t node_type)
{
  ...
  setSpeedAndPowerNRF24L01(2, (3<<1)); // 2 Mbps, max snaga (0 dBm)
...
//// Isključi Auto-ACK (za test bez RX)
//conRegNRF24L01(NRF24L01_EN_AA, 0x00);	// WARN: ovo je samo za test izbrisati kasnije
...
void setSpeedAndPowerNRF24L01(uint8_t dr, uint8_t pwr)
{
    uint8_t reg = 0;

    if (dr == 2) {              // 2 Mbps
        reg |= (1 << 3);        // RF_DR_HIGH
    } else if (dr == 250) {     // 250 kbps
        reg |= (1 << 5);        // RF_DR_LOW
    } else {                    // default 1 Mbps
        // oba bita 0
    }

    reg |= (pwr & 0x06);        // RF_PWR[2:1] → snaga
    conRegNRF24L01(NRF24L01_RF_SETUP, reg);
}
