...
// NOTE: ovu funk pozvati u DCMI funk.
//void initDMA2_for_OV7670(uint32_t* buff0,uint32_t* buff1,uint32_t tsize)
//{
//  // DMA2 setup
//  {
//    RCC->AHB1ENR |= RCC_AHB1ENR_DMA2EN;
//    // DMA2_Stream1 -> se nalazi DCMI -> channel 1
//    DMA2_Stream1->CR = 0x00000000;      // DISABLE STREAM
//
//    while ((DMA2_Stream1->CR & DMA_SxCR_EN) == DMA_SxCR_EN);  // sacekaj dok DMA transfer ne zavrsi  (kad je 1 znaci radi stream u smislu "radim ne sad")
//    
//    /*
//      NOTE: Before setting EN bit to '1' to start a new transfer, the event flags corresponding to the
//            stream in DMA_LISR or DMA_HISR register must be cleared.
...
//
//
//    DMA2_Stream1->PAR = (uint32_t)&DCMI->DR;                // periferal adres from/to. Adresa od koje zelimo da prenesemo podatke.
//    DMA2_Stream1->M0AR = (uint32_t)buff0;                   // ovo je prvi buffer (memoriska adresa prvog buffera)
//    DMA2_Stream1->M1AR = (uint32_t)buff1;                   // memorijiska adresa drugog buffera
//    DMA2_Stream1->NDTR = tsize;                             // velicina, numbers of data items to transfer OR number of samples to write
//    // WARN: Da li mi treba ovo M1AR i ima li poente koristiti cirkularni buffer na DCMI ??? 
//
//    DMA2_Stream1->CR |= DMA_SxCR_CHSEL_0;                   // 001: channel 1 selected 
//    DMA2_Stream1->CR |= DMA_SxCR_PL;                        // Priority level-> 11: Very high
//    DMA2_Stream1->CR |= DMA_SxCR_MINC;                      // 1: Memory address pointer is incremented after each data transfer 
//                                                            // (increment is done according to MSIZE)
//    
//    DMA2_Stream1->CR |= DMA_SxCR_DBM;                       // double buffer mode 
//    DMA2_Stream1->CR |= DMA_SxCR_CIRC;                      // 1: Circular mode enabled
//
//    DMA2_Stream1->CR |= DMA_SxCR_PSIZE_0;                   // Peripheral data size-> 01: Half-word (16-bit)
//    DMA2_Stream1->CR |= DMA_SxCR_MSIZE_0;                   // Memory data size-> 01: Half-word (16-bit)
//    
//    DMA2_Stream1->CR |= DMA_SxCR_TCIE;                      // Transfer Complete Interrupt
//
//
//    DMA2_Stream1->CR &= ~DMA_SxCR_DIR;                      // Data transfer direction-> 00: Peripheral-to-memory
//
//    DMA2_Stream1->CR |= DMA_SxCR_EN;                        // Stream enable / flag stream ready when read low
//    
//    
//    
//  }
...
//void initDMA2_for_OV7670_snapshot(uint32_t* buff0, uint32_t tsize_words)
//{
//    RCC->AHB1ENR |= RCC_AHB1ENR_DMA2EN;
//
...
//    DMA2_Stream1->NDTR = tsize_words;   // broj 32b riječi, tj. pola piksela
//
//    // Channel 1, very high prio, inc mem, 32b/32b, periph-to-mem
//    DMA2_Stream1->CR |= (1 << 25);            // CHSEL = 1 (DCMI)
...
//    DMA2_Stream1->CR |= DMA_SxCR_DIR_0;       // periph->mem
//    DMA2_Stream1->CR |= DMA_SxCR_TCIE;        // interrupt
//
//}



//void initDMA2_for_OV7670_snapshot(uint16_t* buff0, uint32_t tsize_words)
//{
//    RCC->AHB1ENR |= RCC_AHB1ENR_DMA2EN;
//
//    DMA2_Stream1->CR = 0x00000000;
...
//    DMA2_Stream1->PAR  = (uint32_t)&DCMI->DR;
//    DMA2_Stream1->M0AR = (uint32_t)buff0;
//    DMA2_Stream1->NDTR = tsize_words;   // broj 16-bit riječi (jedan pixel po elementu)
//
//    DMA2_Stream1->CR |= (1 << 25);       // CHSEL = 1 (DCMI)
//    DMA2_Stream1->CR |= DMA_SxCR_PL_1 | DMA_SxCR_PL_0; // very high priority
//    DMA2_Stream1->CR |= DMA_SxCR_MINC;   // increment memory
//
//    // PSIZE = 16b, MSIZE = 16b
//    DMA2_Stream1->CR &= ~DMA_SxCR_PSIZE;
//    DMA2_Stream1->CR &= ~DMA_SxCR_MSIZE;
//
//    DMA2_Stream1->CR |= DMA_SxCR_DIR_0;  // periph->mem
//    DMA2_Stream1->CR |= DMA_SxCR_TCIE;   // interrupt
//}

/*    WARN: wratiti se na ovu funk. kada napravim za cirkularni buffer
 *
void DMA2_Stream1_IRQHandler(void)
{
    if (DMA2->LISR & DMA_LISR_TCIF1)  // Transfer complete flag for Stream1
    {
        DMA2->LIFCR = DMA_LIFCR_CTCIF1;  // Clear flag
        
        // ---> Ovdje setuj neku globalnu varijablu da javi main petlji da je frame spreman
    }
}
*/
