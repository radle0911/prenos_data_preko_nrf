...
// WWWWWWWWWWWWWWWWWW ZA SPI3 WWWWWWWWWWWWWWWWWW
...
// _________________________________ OVO je za 1. NRF ____________________
...
uint8_t txDataNRF24L01(uint8_t *daddr, uint8_t *data)    // data_address, data
{
  ...
  while (txStatusNRF() == NRF_TX_IN_PROGRESS) {	
    if (chk4TimeoutSYSTIMER_TIM7(tajmer, NRF24L01_TX_WAIT_PERIOD) == SYSTIMER_TIMEOUT) {   // ako se ne zavrsi za 100ms fail je 
...
//
void txContCarrierNRF24L01(uint32_t period, uint16_t ch) 
{
  uint8_t data[4];
...
// WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW NRF sa SPI3 WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW
...
void initnRF24L01_SPI3(uint8_t node_type)
{
  ...
  //wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww
  // PD6 - nRF24L01 CE
  // PD7 - nRF24L01 CS	  -> SPI3 podeseno
  // PB3 - nRF24L01 CLK	  -> SPI3 podeseno
  // PB4 - nRF24L01 MISO  -> SPI3 podeseno
  // PB5 - nRF24L01 MOSI  -> SPI3 podeseno
  //---------------------------------------------------------------------	
  ...
  initSPI3_nRF24lO1(SPI_BaudRate_Prescaler_16);	
  ...
  //---------------------------------------------------------------------------------------------
  NRF_CE_LOW_SPI3;     // CE = LOW = “ne radi ništa, čekaj da ti sve podestim”. 
  // Ako je CE HIGH dok podešavaš registre, čip može biti usred prijema/slanja
  // i ignorisati ili pogrešno primiti SPI komande.
  // Zato se uvijek drži CE = LOW dok traje inicijalizacija, a digneš ga tek 
  // kad si sve podesio i spreman za rad.
  ...
  //---------------------------------------------------------------------------------------------
  //  CS (ili CSN) = chip select. Aktivno stanje mu je LOW (negativna logika).
  SPI3_CS_HIGH;   //  CSN = HIGH = “ne pričam s tobom dok ne budem spreman”.
  //---------------------------------------------------------------------------------------------
  delay_ms(100);  // Razlog:
  // 
  // Nakon napajanja ili resetovanja, nRF24L01-u treba Tpd2stby vrijeme iz datasheet-a 
  ...
  */
  ...
  conRegNRF24L01_SPI3(NRF24L01_RX_PW_P0, NRF24L01_PIPE_LENGTH_3);			// set length of pipe 0
  conRegNRF24L01_SPI3(NRF24L01_RX_PW_P1, NRF24L01_PIPE_LENGTH_3);			// set length of pipe 1
...
{
  NRF_CE_LOW_SPI3;
  setRegNRF24L01_SPI3(reg, addr, 5);
  NRF_CE_HIGH_SPI3;
}
...
{
  setRegNRF24L01_SPI3(reg, &reg_val, 1);
}
...
{/// Rx mode, power up the nRF TRx, CRC One byte!
	...
	conRegNRF24L01_SPI3(NRF24L01_CONFIG,((1<<NRF24L01_CONFIG_EN_CRC)|(0<<NRF24L01_CONFIG_CRCO))|((1<<NRF24L01_CONFIG_POWER_UP)|(1<<NRF24L01_CONFIG_PRIM_RX)));
	NRF_CE_HIGH_SPI3;
	delay_us(130);	
	conRegNRF24L01_SPI3(NRF24L01_STATUS, (1<<NRF24L01_STATUS_TX_DS)|(1<<NRF24L01_STATUS_MAX_RT));
}
...
{ // mora biti isto u primarno P0 i TX radi ACK
  setRegNRF24L01_SPI3(NRF24L01_RX_ADDR_P0, daddr, 5);
  setRegNRF24L01_SPI3(NRF24L01_TX_ADDR, daddr, 5);
}
...
void txPacketNRF24L01_SPI3(uint8_t* data)
{
  ...
  while (nrf_mode == NRF_MODE_TX) {
    ...
    status = getRegNRF24L01_SPI3(NRF24L01_STATUS);				    // Trebamo provjeriti da li je promjenjen nrf_mode
    if (status & ((1 << NRF24L01_STATUS_TX_DS) | (1 << NRF24L01_STATUS_MAX_RT))) {  // ovi biti se ciste upisom 1 bit-a
      nrf_mode = NRF_MODE_RX;
      break;
    }

    // pravimo delay od 1s i vrsimo ispis nakon 1s
    if (chk4TimeoutSYSTIMER_TIM7(tajmer, 1000) == SYSTIMER_TIMEOUT) {  // 1ms
      printUSART2("TIMEOUT\n");
      return;
    }
    ...
    NRF_CE_LOW_SPI3;	  // ------------------
    setTxModeNRF24L01_SPI3();												// set Tx mode
    flushTxNRF24L01_SPI3();
    SPI3_CS_LOW;

    tmp = NRF_W_TX_PAYLOAD; // SPI commands
    txSPI3_nRF(&tmp, 1);    // select w_tx_payload, clear ss bit 
    txSPI3_nRF(data, NRF24L01_PIPE_LENGTH_3);
...
uint8_t txStatusNRF_SPI3(){
  uint8_t status;
  if (nrf_mode == NRF_MODE_TX) {  // prvo provjeravamo da li je mode uopste podesen za TX
    status = getRegNRF24L01_SPI3(NRF24L01_STATUS); // adresa statusa, uzimamo sve inf. iz nje ovako
    if (status & ((1 << (NRF24L01_STATUS_TX_DS)) | (1 << (NRF24L01_STATUS_MAX_RT)))) 
    {
      setRxModeNRF24L01_SPI3(); // zasto se prebacuje u Rx mode ??? --> AUTO-ACK
      /*Zašto se nakon slanja prebacuje u RX mode?
	Kod nRF24L01, kad pošalješ paket u TX modu:
...
uint8_t txDataNRF24L01_SPI3(uint8_t *daddr, uint8_t *data)    // data_address, data
{
  ...
  while (txStatusNRF_SPI3() == NRF_TX_IN_PROGRESS) {	
    if (chk4TimeoutSYSTIMER_TIM7(tajmer, NRF24L01_TX_WAIT_PERIOD) == SYSTIMER_TIMEOUT) {   // ako se ne zavrsi za 100ms fail je 
      rezultat = NRF24L01_TX_FAILED;
      break;
    }
...
{
  uint8_t status = getRegNRF24L01_SPI3(NRF24L01_STATUS);
  if (status & (1 << (NRF24L01_STATUS_RX_DR))) {
    status = NRF_DATA_READY;
  }else {
    // uzimamo RX FIFO status
    status = getRegNRF24L01_SPI3(NRF24L01_FIFO_STATUS);
    status &=  (1 << NRF24L01_FIFO_STATUS_RX_EMPTY);
    if (status) {
      status = NRF_DATA_NOT_READY;
    }else {
...
void rxDataNRF24L01_SPI3(uint8_t * data)
{
  ...
  tmp = (NRF_R_RX_PAYLOAD);
  txSPI3_nRF(&tmp, 1);		  // transmit command to read RX payload
  rxSPI3_nRF(data, (NRF24L01_PIPE_LENGTH_3));
  SPI3_CS_HIGH;
  
  conRegNRF24L01_SPI3(NRF24L01_STATUS,(1<<(NRF24L01_STATUS_RX_DR)));
...
{ 
  ...
  SPI3_CS_LOW;
  txSPI3_nRF(&registar, 1);
  txSPI3_nRF(reg_value, n_value);
  SPI3_CS_HIGH;                               // n_value => predstavlja velicinu adrese tipa RX_ADDR_P0 ima max 5Bayta 40bita [39:0]
                                              // ovi podaci se nalaze u nRF->manual
}
